...
namespace JsonMapping.JSONMapper
{
    public static class JSONMapper
    {

        public static IEnumerable<string> WildcardsJPaths = JsonFieldFinder.JsonFieldFinder.GetJPaths(10);

        public static string Map2Requested(EventObject eventObject)
        {
            ...
            var listOfParentObject = JsonFieldFinder.JsonFieldFinder
                .FindFieldsIEnumerable("event.primaryobject", jsonCollection.Find(x => x.Key == "eventmap").Value.ToString(), WildcardsJPaths);
        ...
        private static (bool required, bool remove, object fieldvalue) getFieldValue(
            JObject Definitions, string path, List<KeyValuePair<string, string>> jsonCollection, EventObject eventObject, JToken populatedObject)
        {
            ...
            if (eventObject.dataPayloadKeywords.Any(x => x.Key == datapayload))
            {
                KeyValuePair<string, string> dataPayloadType = eventObject.dataPayloadKeywords.FirstOrDefault(x => x.Key == datapayload);
        ...
        private static (bool required, bool remove, object fieldvalue) getFieldValue(
            JObject Definitions, string path, List<KeyValuePair<string, string>> jsonCollection, EventObject eventObject)
        {
            /// access the eventObject for instantiating the methods for expressions
            /// Parse the Field Definitions
            var datapayload = ((JValue)Definitions
        ...
        /// <returns></returns>
        private static object Map2RequestedChild(EventObject eventObject, string objectName, List<KeyValuePair<string, string>> jsonCollection)
        {
            /// Get Event Map
            var eventMap = JObject.Parse(jsonCollection.Find(x => x.Key == "eventmap").Value.ToString());
            ...
            (bool required, bool deleteifnull, object fieldvalue) fieldValue;
            /// Hydrate the Parent Object Structure
            foreach (var ea in fieldsOfParentObject.Children().ToArray().AsParallel())
            {
        ...
        /// <param name="objectName"></param>
        /// <returns></returns>
        private static object Map2RequestedChildren(EventObject eventObject, DefinitionsObject sourceDefinitions, List<KeyValuePair<string, string>> jsonCollection)
        {
            /// Get Event Map
            var eventMap = JObject.Parse(jsonCollection.Find(x => x.Key == "eventmap").Value.ToString());
            ...
            foreach (var ea in listOfJsonObjectsFromPayload.Children().ToArray().AsParallel())
            {
