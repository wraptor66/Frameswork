...
namespace JsonMapping.JsonFieldFinder
{
    public static class JsonFieldFinder
    {
        public static IEnumerable<string> GetJPaths(int levels)
        {
            ...
            foreach (var ea in filterPerms.AsParallel())
            {
                sb.Clear().Append(ea);
                actualPath = sb
                     .Replace("0", "*")
        ...
        public static IEnumerable<JToken> FindFieldsIEnumerable(string fieldname, string payload, IEnumerable<string> filterPermutations)
        {
            try
            {
                ...
                foreach (var ea in filterPermutations.AsParallel())
                {
        ...
        //((JProperty)token.First).Value
        /// <summary>
        /// 
        /// </summary>
        /// <param name="fieldname">parent.fieldname</param>
        /// <param name="payload"></param>
        /// <param name="filterPermutations"></param>
        /// <returns></returns>
        public static object FindFieldValue(string fieldname, JObject payload, IEnumerable<string> filterPermutations)
        {
            try
            {
                string wildcardPath = string.Empty;
                ...
                {
                    wildcardPath = ea + fieldname;
                    var _sTokens = payload.SelectTokens(wildcardPath);
                    if (_sTokens.ToList().Count > 0)
                    {
                        bool exists = false;
                        //write the found to a list 
